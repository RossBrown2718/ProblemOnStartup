...
public class DifferentiationGenerator extends JFrame implements KeyListener {
...
//	private boolean advancedTrig;
	...
	public DifferentiationGenerator() {
//		power = p1;
//		product = p2;
//		quotient = q;
//		chain = c;
	...
	public void createAndDifferentiate(int type) {
		...
		if (type == 0) { // TODO change to 0
			...
			if (type1 == 0) {
				...
				problem += typeTrig == 0 ? "tan(x)*"
						: typeTrig == 1 ? "cot(x)*" : typeTrig == 2 ? "sec(x)*" : "csc(x)*";
				answer += typeTrig == 0 ? "sec^2(x)*"
						: typeTrig == 1 ? "csc^2(x)*" : typeTrig == 2 ? "tan(x)*sec(x)*" : "cot(x)*csc(x)*";
			...
			if (type2 == 0) {
				...
				if (sine == 1)
					neg2 = !neg2;
				problem += sine == 0 ? "sin(x)" : "cos(x)";
				answer += (sine == 0 ? "sin(x)" : "cos(x)") + (neg2 ? "-" : "+") + tempAns
						+ (sine == 0 ? "cos(x)" : "sin(x)");
			} else if (type2 == 3) {
				int typeTrig = (int) (Math.random() * 4); // 0 is tan, 1 is cot, 2 is sec, 3 is csc
				if (typeTrig % 2 == 1)
				...
				answer += (typeTrig == 0 ? "tan(x)" : typeTrig == 1 ? "cot(x)" : typeTrig == 2 ? "sec(x)" : "csc(x)")
						+ (neg2 ? "-" : "+") + tempAns + (typeTrig == 0 ? "sec^2(x)"
								: typeTrig == 1 ? "csc^2(x)" : typeTrig == 2 ? "tan(x)*sec(x)" : "cot(x)*csc(x)");
			}
			...
			if (type2 == 0) {
				...
				answer += (typeTrig == 0 ? "sec^2" + tempAns
						: typeTrig == 1 ? "csc^2" + tempAns
								: typeTrig == 2 ? "tan" + tempAns + "*sec" + tempAns
										: "cot" + tempAns + "*csc" + tempAns);
			}
			answer = neg ? "-" + answer : answer;
		}
